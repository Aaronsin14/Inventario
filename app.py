...
DATABASE_URL = os.getenv("DATABASE_URL") or \
    "postgresql://inventario_user:VG0AF852QrAB0xMr9lRWlyWnpybQBTNA@dpg-d6nhomh5pdvs73bin8og-a.oregon-postgres.render.com/inventario_4oa6"

def get_conn():
    """Crea una conexión nueva a la DB por cada request"""
    ...
    try:
        ...
        with get_conn() as conn:
            with conn.cursor() as cursor:
                ...
                nombre, precio_real, stock = row[0], float(row[1]), int(row[2])
                if cantidad > stock:
                ...
                cursor.execute("""
                    INSERT INTO ventas (producto_id,nombre_producto,cantidad,precio_unitario,total)
                    VALUES (%s,%s,%s,%s,%s)
                """, (id, nombre, cantidad, precio_final, total))
            conn.commit()
        return jsonify({"mensaje":"Venta realizada"})
    except Exception as e:
        print("Error vender_producto:", e)
        return jsonify({"mensaje":"Error"}), 500
...
def api_dashboard():
    try:
        with get_conn() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    SELECT DATE_TRUNC('week',fecha), SUM(cantidad), SUM(total)
                    FROM ventas
