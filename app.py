...
with conn.cursor() as cursor:
    ...
    if cursor.fetchone()[0] == 0:
        ...
        ('vendedor2','venta123','vendedor'),
        ('vendedor3','venta123','vendedor')
        """)
...
def proteger_rutas():
    ...
    if any(request.path.startswith(p) for p in publicas):
        return
    if "usuario" not in session:
        return redirect("/")
    if request.path.startswith("/dashboard") and session.get("rol") != "admin":
...
def login():
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("""
            SELECT id,rol,password FROM usuarios
            WHERE usuario=%s
            """,(usuario,))
            row = cursor.fetchone()
        ...
        print("ERROR LOGIN:",e)
        return jsonify({"mensaje":"Error de servidor"}),500
